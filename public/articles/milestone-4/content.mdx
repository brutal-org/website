...
It's been a long time, 450 commits to be precise but we are back with a new milestone for the BRUTAL operating system! This one focused on the desktop side of things, but also on improving our IPC framework and init system.
...
You might think this is a bit overkill for a simple blog, but we have big plans for the future ;)
...
Here is an exemple of an IPC module describing a simple event dispatcher:
...
Based on the improved IPC framework and graphic library, we built a simple widget library with support for buttons, labels, windows, etc...
...
On top of everythings stated above, we fixed countless bugs and improved the overall performance of the operating system. We continued working on our C compiler. Implemented basic support for ext2 filesystems.
